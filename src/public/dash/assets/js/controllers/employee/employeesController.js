function employeesController() {
    ...
    this.filterView = function () {
        ...
        $.each(self.employees, function (key, employee) {
            ...
            if (show) {
                $('#emplpyees_table tbody tr[data-id="' + employee.userId + '"]').show();
            }
        })
    ...
    this.addEmployee = function (employee) {
        if (window.location.pathname == "/dash/employees/employees/") {
            ...
            var listOfButtons = [];
            $('#emplpyees_table tbody').append(`<tr data-id="${employee.userId}" data-type="${employee.companyPosition}" onclick="emplC.employeeCard(${employee.userId})">
            ...
            if (self.showDeactivated || (!self.showDeactivated && employee.active))
                $('select.employee').append('<option value="' + employee.userId + '">' + employee.userName + '</option>');
        } else if (window.location.pathname == "/dash/employees/working_schedule/" || window.location.pathname == "/dash/resellers/employees_working/" || window.location.pathname == "/dash/working_schedule/") {
            if (!employee.active) {
                return false;
            }
            ...
            $('.user_working_shedule ul').append(`<li user-id="${employee.userId}" user-position="${employee.companyPosition}">
    ...
    this.changeSuperSettings = function (el) {
        ...
        AjaxController('updateEmployeeRights', {userId: self.curEmployee.userId, param: param, val: val}, adminUrl, emplC.updateEmployeeHandler, errorBasicHandler, true)
    }
    ...
    this.payEmployeeHandler = function (response) {
        ...
        self.employeeCard(response.data.userId)
        self.setTotal();
    }
    this.payEmployee = function (el) {
        var userId = self.curEmployee.userId;
    ...
    this.updateEmployeeHandler = function (response) {
        ...
        var $userRow = $('#emplpyees_table tr[data-id="' + user.userId + '"]').find('td');
