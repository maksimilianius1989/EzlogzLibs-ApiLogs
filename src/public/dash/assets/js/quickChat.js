$().ready(function ()
{
    $('body').on('click', '#quickChatContactsListHeader', quickChatC.showQuickChatsContactList);
    $('body').on('click', '.oneQuickChatBoxContactList', quickChatC.showOneQuickChatRoom);
    $('body').on('click', '#oneQuickChatRoomHeaderClose', function (e)
    {
        e.stopPropagation();
        $('#oneQuickChatRoom').attr('data-chatid', '').hide();
    });
    $('body').on('click', '#oneQuickChatRoomAddFile', function ()
    {
        $('#quickChatUploadFile').click();
    });
    $('body').on('change', '#quickChatUploadFile', quickChatC.quickChatUploadFile);
    $('body').on('click', '#oneQuickChatRoomCancelFile', function ()
    {
        $('#oneQuickChatRoomAttachmentFile').hide();
        $('#oneQuickChatRoomControlsBox').show();
    });
    $('body').on('click', '#oneQuickChatRoomSmiles', function ()
    {
        $('#quickChatSmilesBox').toggle();
    });
    $("#oneQuickChatRoomMessageBox").scroll(function ()
    {
        if ($(this).scrollTop() == 0)
        {
            if (!gettingMore && !$('.oneQuickChatBoxContactList[id="' + $('#oneQuickChatRoom').attr('data-chatid') + '"]').hasClass('loaded'))
            {
                quickChatC.getMoreQuickChatMessages($('#oneQuickChatRoom').attr('data-chatid'), $('.oneQuickChatMessageBox:not(.quickChatSystemMessageDateTime):first').attr('data-time'));
            }
        }
    });
    $('body').on('keyup', '#oneQuickChatRoomInputField', function (e)
    {
        if (e.which == 13 && $(this).text() != '')
        {
            quickChatC.findSmileObjectQuickChat();
            $('#quickChatSmilesBox').hide();
            quickChatC.sendQuickChatMessage($('#oneQuickChatRoomInputField').text().trim(), 1, $('#oneQuickChatRoom').attr('data-chatid'));
        }
    });
    $('body').on('click', '#quickChatSmilesBox img', function ()
    {
        $('.input_pl').remove();
        quickChatC.convertSmileQuickChat($(this));
    });
    $('body').on('click', '#oneQuickChatRoomInputField', function ()
    {
        $('.input_pl').remove();
        $('#oneQuickChatRoomInputField').attr('contenteditable', true);
        $('#oneQuickChatRoomInputField').focus();
    });
    $('body').on('focusout', '#oneQuickChatRoomInputField', function ()
    {
        $('#oneQuickChatRoomInputField').attr('contenteditable', false);
        quickChatC.messagesBuffer($('#oneQuickChatRoom').attr('data-chatid'), $(this).text());
        if ($(this).text() == '')
        {
            quickChatC.setInputPlaceholderQuickChat();
        }
    });
    $('body').on('click', '#oneQuickChatRoomSendFile', function ()
    {
        $('#oneQuickChatRoomSendFile').prop('disabled', true);
        quickChatC.sendQuickChatMessage($('#oneQuickChatRoomAttachmentFile').attr('data-src'), $('#oneQuickChatRoomAttachmentFile').attr('data-type'), $('#oneQuickChatRoom').attr('data-chatid'));
    });
    $('body').on('click', '.quickChatMessageImage', function ()
    {
        var img = new Image();
        img.onload = function ()
        {
            $('#quickChatAttachmentBox').show();
            $('#quickChatShowFile').css({'max-height': $(window).height() * 0.9 - 100});
            $('#quickChatShowFile').css({'height': img.height});
            $('#quickChatShowFile').empty().append(img);
        };
        img.src = $(this).attr('src');
    });
    $('body').on('click', '.quickChatMessageVideo, .playVideoButton', function ()
    {
        $('#quickChatShowFile').empty().append('<video controls><source src="' + $(this).parents('.oneQuickChatMessageBox').find('source').attr('src') + '"></video>');
        $('#quickChatShowFile video')[0].addEventListener("loadedmetadata", function ()
        {
            $('#quickChatAttachmentBox').show();
            $('#quickChatShowFile').css({'max-height': $(window).height() * 0.9 - 100});
            $('#quickChatShowFile').css({'height': this.videoHeight});
            $('#quickChatShowFile video')[0].play();
        }, false);
    });
    $('body').on('click', '.quickChatMessageLocation', function ()
    {
        $('#quickChatAttachmentBox').show();
        $('#quickChatShowFile').empty().append('<div id = "quickChatLocation"></div>');
        $('#quickChatShowFile').css({'height': $(window).height() * 0.9 - 100});
        $('#quickChatLocation').css({'width': $(window).width() * 0.9 - 100});
        quickChatC.showQuickChatLocationMessage($(this).attr('data-lat'), $(this).attr('data-long'));
    });
    $('body').on('click', '.quickChatMessageContact', function ()
    {
        var userId = $(this).attr("data-userid");

        if (quickChatC.quickChatCheckUserAllReadyInConatacts(userId))
        {
            $('#quickChatAttachmentBox').show();
            $('#quickChatShowFile').empty().append('<div id = "quickChatAddContact">\n\
                <p>Add to contacts?</p>\n\
                <button id="quickChatAddContactTrue" data-userid="' + userId + '">Ok</button>\n\
                <button id="quickChatAddContactFalse">Cansel</button>\n\
            </div>');
        }
    });
    $('body').on('click', '#quickChatAddContactFalse', function ()
    {
        $('.close_edit').click();
    });
    $('body').on('click', '#quickChatAddContactTrue', function ()
    {
        quickChatC.quickChatAddUserToContacts($(this).attr('data-userid'));
        $('.close_edit').click();
    });
    $('body').on('click', '.close_edit', function ()
    {
        $('#quickChatShowFile').empty();
    });
    $("#oneQuickChatRoomInputField").on('paste', quickChatC.getClipboardInfo);
    $(document).bind("mousedown", (e) =>
    {
        if (!$(e.target).parents("#quickChatSmilesBox").length > 0 && $(e.target).attr('id') != "quickChatSmilesBox" && $(e.target).attr('id') != "oneQuickChatRoomSmiles" &&
                !$(e.target).parents("#quickChatShowFile").length > 0 && $(e.target).attr('class') != "popup_box_panel" && $(e.target).attr('id') != "quickChatShowFile" &&
                !$(e.target).parents(".quickChatMessageLocation").length > 0 && $(e.target).attr('class') != "quickChatMessageVideo")
        {
            $('#quickChatSmilesBox, #quickChatAttachmentBox').hide();
            $('#quickChatShowFile').empty();
        }
    });
});
/*function showOneQuickChatRoom(id = null)
{
    ...
    chats.forEach(function (chat)
    {
        if (chat.id == chatId)
        {
            chatRoomMessages = getOneQuickChatRoomMessages(chat.chat_messages);
            chatInfoContent = getQuickChatInfo(chat);

            chatInfoContent.notReadMessages > 0 ? quickChatReadMessages(chat) : '';
    ...
    setInputPlaceholderQuickChat(chatId);
    ...
    {
        getMoreQuickChatMessages(chatId, $('.oneQuickChatMessageBox:not(.quickChatSystemMessageDateTime):first').attr('data-time'));
}
}
function getOneQuickChatRoomMessages(chatMessages, sendMessage)
{
    ...
    messages.forEach(function (oneMessage, i)
    {
        ...
        switch (oneMessage.type)
        {
            case 1:
                ...
                userMessage = convertSmileQuickChat($('<div></div>').text(oneMessage.message), 'convert');
                break;
            case 2:
        ...
        {
            previewInfo = getLinkPreviewInfo(oneMessage.preview, 2);
        }
...
function updateQuickChatsContactList()
{
    if ($('#collectionQuickChatsBox').hasClass('visibleQuickChatsBox'))
    {
        showQuickChatsContactList(1);
    }
}
function showQuickChatsContactList(updateContactList)
{
    ...
    {
        $('#quickChatContactsListShowHideButton').html('<i class="fa fa-times" aria-hidden="true"></i>').removeClass('quickChatContactsListShow');
        contactListInfo = getQuickChatsContactList();
...
function getQuickChatsContactList()
{
    ...
    {
        if (chat.type == 2) {
            chatsContactListContent += getOneQuickChatBoxInContactList(chat);
...
function getOneQuickChatBoxInContactList(chat)
{
    ...
    {
        quickChatUserLastVisit = getUserLastVisitQuickChat(chat.lastVisit);
    }
    ...
    return '<div id = "' + chat.id + '" class = "oneQuickChatBoxContactList" data-chatname = "' + chatInfo.chatName + '" data-newmes = "' + chat.notReadMes + '">' +
            ...
            '<div class = "oneQuickChatBoxContactListRightBox">\n\
                    ...
                    <div class = "lastQuickChatMessage">' + convertSmileQuickChat($('<div></div>').text(lastChatMessage), 'convert') + '</div>\n\
...
function getQuickChatInfo(chat)
{
...
function convertSmileQuickChat(smile)
{
    if (arguments.length == 1)//отправить смайл в чате
    {
    ...
    } else if (arguments.length == 2)//конвертировать сообщ в смайл
    {
        return emojione.toImage($(smile).text());
    }
}*/
//function sendQuickChatMessage(message, type, chatId)
//{
//    if (type == 1)
//    {
...
//}
/*function quickChatReadMessages(chat)
{
    var data = {
...
function setInputPlaceholderQuickChat(chatId)
{
    var unfinishedMessageText = messagesBuffer(chatId);
...
function findSmileObjectQuickChat()
{
...
function showQuickChatLocationMessage(lat, long)
{
...
function quickChatAddUserToContacts(userId)
{
    var data = {
        action: 'createChat',
        data: {
...
function quickChatCheckUserAllReadyInConatacts(userId)
{
    var chats = dcc.chats;
...
function getMoreQuickChatMessages(chatId, time) {
    var data = {
...
function getClipboardInfo(event)
{
...
function getUserLastVisitQuickChat(time)
{
