function managerReturnCard(returnId, params = {}) {
    var self = this;
    //changable part
    self.cntrlUrl = adminUrl;
    self.returnId = returnId;
    self.tableId = 'managerReturnCard_' + self.returnId;
    self.modalId = 'manager_return_modal_card';
    self.modalTitle = 'Return ELD Info #' + self.returnId;
    ...
    self.generateHeaders = function () {
        ...
        if (self.returnData.status === 4 && typeof self.returnAdditionalParams.new_scanner === 'undefined') {
            if (self.returnData.returnReason === 1) {
                headers.push({label: 'Bind Info', value: 'Bind new replacement device Mac. Broken device replacement due is $199.99'});
            } else if (self.returnData.returnReason === 2) {
                headers.push({label: 'Bind Info', value: 'Bind new replacement device Mac'});
            }
        }
    ...
    self.generateButtons = function () {
        ...
        if ((self.returnData.status === 1 || self.returnData.status === 3 || self.returnData.status === 4) && (self.returnData.returnReason === 1 || self.returnData.returnReason === 2) && typeof self.returnAdditionalParams.new_scanner == 'undefined') {
            buttons.push(`<button class="btn btn-default" onclick="bindMacAddressReplacementDevice(${self.returnData.returnReason}, ${self.returnData.id})">Bind New mac</button>`);
        }
        ...
        self.tabs.push({
            ...
            cl: 'return_eld_his',
            request: 'getEldReturnHistoryModalPagination',
            handler: 'getEldReturnHistoryModalPaginationHandler',
            tableHeader: `<tr>
                <th>Date Time</th>
                <th>Scanner ID</th>
                <th>User</th>
        ...
        self.tabs.push({
            ...
            handler: 'getManagerSupportReturnCommentHandler',
            tableHeader: `<tr>
                <th>Date Time</th>
                <th>Scanner ID</th>
                <th>User</th>
    ...
    self.getEldReturnHistoryModalPaginationHandler = function (response) {
        var tbody = '';
    ...
    self.getManagerSupportReturnCommentHandler = function (response) {
        var tbody = '';
        response.data.result.forEach((item) => {
            var isComment = item.supportComment !== null ? '- Updated by Support' : '';
            tbody += `<tr>
...
function actionGetOneManagerReturnInfo(el) {
    var id = 'null';
    if ($.isNumeric(el)) {
        id = el;
    } else {
...
function refreshReturnHandler(response) {
    ...
    new managerReturnCard(response.data.returns.id);
    if(typeof pagination !== 'undefined') {
        pagination.request();
    }
}

function bindMacAddressReplacementDevice(returnReason, returnId) {
    ...
    showModal('Bind Mac', body, 'bind_mac_modal', '', {footerButtons: `<button class="btn btn-default" id="bind_button" onclick="generateReplacementDevice(${returnReason}, ${returnId})">Bind</button>`})
    findFreeDeviceMac();
}
function generateReplacementDevice(returnReason, returnId) {
    var macAddressId = $('#select_mac_input').attr('data-id');
    AjaxController('generateReplacementDevice', {returnReason: returnReason, returnId: returnId, newMacBindId:macAddressId}, adminUrl, refreshReturnHandler, errorBasicHandler, true);
}
